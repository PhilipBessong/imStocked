...
export class ItemService {
  ...
  private items = new BehaviorSubject<Item[]>([
  {
    ...
    purchase: 30,
    closing: 35,
    total: 55,       // purchase + physical
    actual: 35,      // total - sale
    variance: 0,     // closing - actual
    dateMade: '2025-07-07',
    entries: [],
  },
  {
 ...
 /* registerUser(newUser: User): void {
  ...
  demoGetItems(): Observable<Item[]> {
    return this.items.asObservable();
  }
  demogetItemByCode(code: string): Observable<Item | undefined> {
  return this.items.asObservable().pipe(
    map(items => items.find(item => item.code === code))
  );
}
